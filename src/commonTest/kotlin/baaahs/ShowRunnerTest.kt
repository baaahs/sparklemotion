...
class ShowRunnerTest {
    ...
    private val surface1Receiver =
        SurfaceReceiver(UnmappedSurface(BrainId("brain1"))) { buffer -> surface1Messages.add(buffer) }
    private val surface2Messages = mutableListOf<Shader.Buffer>()
    private val surface2Receiver =
        SurfaceReceiver(UnmappedSurface(BrainId("brain2"))) { buffer -> surface2Messages.add(buffer) }
