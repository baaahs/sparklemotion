...
interface MutableEditable<T> {
    val title: String
    fun getEditorPanels(editableManager: EditableManager<T>): List<DialogPanel>
}

interface EditIntent<T> {
    fun findMutableEditable(mutableDocument: MutableDocument<T>): MutableEditable<T>
     ...
     */
    fun refreshEditIntent(): EditIntent<T> = this
     ...
     */
    fun nextEditIntent(): EditIntent<T> = this
}

class ShowEditIntent : EditIntent<Show> {
    override fun findMutableEditable(mutableDocument: MutableDocument<Show>): MutableEditable<Show> =
        mutableDocument
}

data class ControlEditIntent(internal val controlId: String) : EditIntent<Show> {
    ...
    override fun refreshEditIntent(): EditIntent<Show> {
        return copy(controlId = mutableEditable.asBuiltId!!)
    }

    override fun nextEditIntent(): EditIntent<Show> {
        return ControlEditIntent(mutableEditable.asBuiltId!!)
    }
}

abstract class AddToContainerEditIntent<T: MutableControl> : EditIntent<Show> {
