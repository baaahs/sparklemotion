...
object LifeyShow : Show("Lifey") {
    override fun createRenderer(model: Model<*>, showContext: ShowContext): Renderer {
        ...
        val speedSlider = showContext.getGadget("speed", Slider("Speed", .25f))

        val shader = SolidShader()
        val shaderBuffers = showContext.allSurfaces.associateWith {
            showContext.getShaderBuffer(it, shader).apply { color = Color.WHITE }
        }
