...
object HeartbleatShow : Show("Heartbleat") {
    override fun createRenderer(model: Model<*>, showContext: ShowContext): Renderer {
        ...
        return object : Renderer {
            val hearts = showContext.allSurfaces.filter { it is IdentifiedSurface && it.number == 7 }
                .map { showContext.getShaderBuffer(it, HeartShader()) }
            val heartSizeGadget = showContext.getGadget("heartSize", Slider("Heart Size", .16f))
            val strokeSize = showContext.getGadget("strokeSize", Slider("Stroke Size", .5f))
            val xOff = showContext.getGadget("xOff", Slider("X Offset", .4f))
            val yOff = showContext.getGadget("yOff", Slider("Y Offset", .67f))
            val otherSurfaces = showContext.allUnusedSurfaces.map { showContext.getShaderBuffer(it, SolidShader()) }

            override fun nextFrame() {
                val currentBeat = showContext.currentBeat
