...
float getfrequency(float x) { return .2 + .2 * beatInfo_intensity;  /* texture(soundAnalysis.buckets, vec2(floor(x * FREQ_RANGE + 1.0) / FREQ_RANGE, 0.25)).x + 0.06; */ }
...
vec3 colorFunction_hsv(vec2 fragPos) {
    vec3 c = vec3( ( atan(fragPos.x, fragPos.y) + time * 0.25 ) / (PI * 2.0), 1.0, 1.0 );
...
vec3 colorFunction_lensFlare(vec2 fragPos) {
    bool VERTICAL_LENS_FLARE = true;
    float lensFlareThickness = 7.;
    float glowFactor = 0.06;
    float perpendicularFalloff = 0.4 * LOGO_RADIUS;
    vec3 baseColor = vec3(1., 1., 1.);
    vec3 highlightColor = vec3(0., 1., 2.);
    ...
    return glowFactor * perpendicularBrightness * (parallel + cos(parallel+highlightColor)+baseColor);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    fragColor = vec4(drawBaaahsLogo(fragCoord, LOGO_RADIUS), 1.0);
}