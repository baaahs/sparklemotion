...
public class TestCoroutineDispatcher(dispatchImmediately: Boolean = false): CoroutineDispatcher(), Delay, DelayController {
    private var dispatchImmediately = dispatchImmediately
        set(value) {
            field = value
            if (value) {
    ...
    private fun doActionsUntil(targetTime: Long, justOne: Boolean = false) {
        while (true) {
    ...
    fun runOne(): Unit = doActionsUntil(currentTime, true)
...
private fun atomic(initialValue: Long): AtomicLong = AtomicLong(initialValue)
