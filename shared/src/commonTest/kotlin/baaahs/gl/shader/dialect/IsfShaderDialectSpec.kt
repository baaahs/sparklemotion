    ...
    describe<IsfShaderDialect> {
        ...
        context("a shader having an ISF block at the top") {
            ...
            it("generates correct color picker feed") {
                ColorPickerFeed.build(openShader.inputPorts[2])
                    .shouldBe(ColorPickerFeed("Fill Color", Color.from("#ff0000")))
            }
            ...
            context("using IMG_PIXEL etc.") {
                ...
                it("includes it as an input port") {
                    ...
                    openShader.inputPorts.last().isAbstractFunction.shouldBe(true)
                }
                ...
                context("linking") {
                    ...
                    it("suggests sensible options") {
                        suggestions.find { it.mutableShader.title == openShader.title }!!
                            .linkOptionsFor("inputImage").stringify()
                            .shouldBe("""
                                Feed:
                                - Date (advanced)
                        ...
                        val linkedProgram by value {
                            autoWireWithDefaults(upstreamShader, openShader.shader,
                                ...
                                editShader(openShader.title) {
                                    // TODO: DO NOT COMMIT: 11/1/22 it should be auto prioritized higher because it's a filter
                                    priority = 1f
                                }
                            }
