...
import baaahs.net.AndroidNetwork.Companion.msgId
...
class AndroidUdpProxy {
    companion object {
        val logger = Logger<AndroidUdpProxy>()

        val broadcastAddresses = AndroidNetwork.getBroadcastAddresses()
    }
    ...
    suspend fun handle(
        ...
        val listenThread = Thread {
            val data = ByteArray(AndroidNetwork.MAX_UDP_SIZE)
            val packetIn = DatagramPacket(data, AndroidNetwork.MAX_UDP_SIZE)
            while (true) {
