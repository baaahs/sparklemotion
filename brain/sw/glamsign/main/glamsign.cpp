...
void
Glamsign::msgBrainPanelShade(Msg* pMsg) {
    if (!pMsg) return;
...
void
Glamsign::msgBrainIdRequest(Msg* pMsg) {
    if (!pMsg) return;
...
void
Glamsign::msgBrainMapping(Msg* pMsg) {
    if (!pMsg) return;
...
void
Glamsign::msgUseFirmware(Msg *pMsg){
    if (!pMsg) return;
...
void
Glamsign::sendPong(const IpPort &port, const uint8_t *data, size_t dataLen) {
...
{
    ((Glamsign*)pvTimerGetTimerID(hTimer))->maybeSendHello();
}

void
Glamsign::maybeSendHello()
{
    if (otaStarted()) {
...
void
Glamsign::sendHello(const IpPort &port) {
...
bool Glamsign::otaStarted() {
    return m_otaStartedAt.tv_sec != 0;
}

void Glamsign::stopEverythingForOTA() {
    m_msgSlinger.stop();
    m_shadeTree.stop();
    m_ledRenderer.stop();
    // m_httpServer.stop();
