...
Wearable::start() {
    ...
    m_shaderManual = new IsoShaderManual(m_state);
...
//    m_ledRenderer.setShader(&m_masterStack);
    ...
    m_shaderManual->setColorCount(m_ledRenderer.getNumPixels());
    uint8_t defColor[] = { 255, 0, 0 };
    for(uint16_t ix=0; ix < m_ledRenderer.getNumPixels(); ix++) {
        m_shaderManual->setColor(ix, defColor);
    }

    m_ledRenderer.setShader(m_shaderManual);
    ...
    m_httpServer.setColorHandlerDelegate(this);
...
uint16_t
Wearable::getColorCount() {
    return m_ledRenderer.getNumPixels();
}

void
Wearable::getColor(int16_t index, uint8_t* value) {
}

void
Wearable::setColor(int16_t index, uint8_t* value) {
    if (index < 0) {
        m_state.setChosenColor((uint8_t)(index * -1) - 1, RgbColor(value[0], value[1], value[2]));
    } else {
        if (m_shaderManual) {
            m_shaderManual->setColor(index, value);
        }
    }
}
